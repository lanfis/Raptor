...
from models.tf_dataset_label_encoder import DATASET_LABEL_ENCODER
...
print("--Resolving dataset structure ...")
tagger.set_dataset_folder(dataset_folder)
tagger.set_index_file(index_file)
tagger.init()
tagger.load_index_file()
    
    

print("--Encoding dataset labels ...")
path_label_list = tagger.encoding_index_list()
#np.random.shuffle(path_label_list)


index_path_tensor_list = tf_util_op.convert_to_tensor(tagger.index_path_list, dtype=tf.string)
index_code_tensor_list = tf_util_op.convert_to_tensor(tagger.index_code_list, dtype=tf.int32)
...
input_channels = 1
output_labels = len(tagger.code_list)
...
with tf.device("/device:{}:0".format("GPU" if use_gpu else "CPU")):
    ...
    print("--label = {}".format(tagger.label_list[int(predict_val)]))
